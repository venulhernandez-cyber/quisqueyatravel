...
## 🔴 SESIÓN — 2026-09-21: auditoría semanal completa (solo lectura, sin cambios en el sitio)

Corrida automática de `auditoria-semanal-quisqueya-travel`. **Confirmado otra vez (como el 7-sep, contradiciendo lo asumido en la corrida del 16-sep): `Desktop\Quisqueya` es la fuente de verdad real**, no `Documentos\GitHub\quisqueyatravel`. Verificado con `git remote`/`git log`: `GitHub\quisqueyatravel` sigue con su rama local divergida de `origin/main` (1 commit propio vs. **122 commits detrás**, último commit local 18-ago — se atrasó más desde el 7-sep) — checkout abandonado. `Desktop\Quisqueya` está "up to date with origin/main" (HEAD `ced61fd`, 19-sep) más cambios locales sin commitear normales de trabajo en curso. Esta auditoría corrió sobre `Desktop\Quisqueya`. Recomendación repetida: dejar de usar la carpeta de `Documentos\GitHub` o resetearla a `origin/main`.

**🔴 HALLAZGO URGENTE — CSP en producción bloquea AdSense, Meta Pixel y los widgets nuevos de Travelpayouts en las 49 guías actualizadas esta semana.** El sitio tiene **tres declaraciones de CSP distintas y desincronizadas**: (1) `_headers` (completa, con todos los dominios), (2) `functions/_middleware.js` (la que de verdad se sirve — confirmado por comentario propio del código, líneas 3-10, que Cloudflare Pages ignora silenciosamente la de `_headers`), y (3) un `<meta http-equiv="Content-Security-Policy">` inyectado en el `<head>` de cada página (ej. `index.html` línea 4, `guia-bayahibe.html` línea 4 — completa, igual de permisiva que `_headers`). **El problema:** cuando hay CSP por header HTTP + CSP por meta tag a la vez, el navegador exige que un recurso cumpla las DOS — no gana la más permisiva, sino que se aplica la intersección. El `script-src` de `functions/_middleware.js` (línea 118) es: `'self' 'unsafe-inline' cdn.jsdelivr.net www.googletagmanager.com emrldtp.com cloud.umami.is` — **le faltan `pagead2.googlesyndication.com` (AdSense, presente en 90 páginas), `connect.facebook.net` (Meta Pixel, `fbevents.js`, en prácticamente todas las páginas), `tpemb.com`/`tp.media` (widgets de Travelpayouts recién agregados a 22 guías el 19-sep) y `unpkg.com`.** Tampoco tiene directiva `frame-src` (fallback a `default-src 'self'`, bloqueando cualquier iframe externo). El commit `31f3bb7` (18-sep) se tituló "sincroniza CSP entre `_headers` y `_middleware.js`" pero solo agregó `cloud.umami.is` — nunca agregó los dominios de Travelpayouts/AdSense/Facebook que sí tiene `_headers`. **Efecto real probable ahora mismo en quisqueyatravel.org:** anuncios de AdSense no cargan (pérdida de ingresos por ads), el Pixel de Meta no dispara eventos (rompe optimización/retargeting de la cuenta de Meta Ads 290012163), y los widgets de traslados/vuelos/carro/eSIM de Travelpayouts agregados esta semana a 22 guías no se renderizan (0 conversión de esa iniciativa). **Fix concreto:** en `functions/_middleware.js` línea 118, agregar a `script-src` y `connect-src`: `https://pagead2.googlesyndication.com https://*.googlesyndication.com https://connect.facebook.net https://tpemb.com https://*.tpemb.com https://tp.media https://*.tp.media https://*.travelpayouts.com https://*.aviasales.com https://*.aviasales.es https://*.aviasales.fr https://*.kiwitaxi.com https://*.airalo.com https://*.getrentacar.com https://*.tpo.li https://unpkg.com https://www.travelpayouts.com`; agregar directiva `frame-src` con `googleads.g.doubleclick.net tpc.googlesyndication.com tpemb.com tp.media travelpayouts.com aviasales.com/.es/.fr kiwitaxi.com airalo.com getrentacar.com tpo.li www.stay22.com`. Después de eso, valdría la pena eliminar la CSP duplicada de `_headers` y del `<meta>` en cada HTML para tener una sola fuente de verdad (la de `_middleware.js`) y no volver a desincronizarse. **Requiere sesión en vivo para editar, probar y desplegar — no se tocó nada en esta corrida de solo lectura.** Recomendado verificar en el navegador (consola, pestaña de errores CSP) apenas se pueda antes de aplicar el fix, para confirmar el alcance exacto.

**🟡 Hallazgo medio — 4 páginas de confianza nuevas, creadas pero sin commitear ni publicar:** `contacto.html`, `privacidad.html`, `sobre-nosotros.html`, `terminos.html` existen en `Desktop\Quisqueya` como archivos **untracked** en git (nunca se les hizo `git add`) — no están en producción todavía. Contenido revisado: SEO limpio (title/meta description/H1/canonical correctos en las 4, sin frases genéricas de IA), pero **no están enlazadas desde el nav/footer de `index.html` ni incluidas en `sitemap.xml`** — hace falta agregarlas a ambos cuando se publiquen. `terminos.html` es particularmente importante: es el aviso de divulgación de afiliados (Booking/Travelpayouts) — vale la pena publicarlo pronto por transparencia/cumplimiento del programa de afiliados. Contraste revisado en las 4: todo bien excepto `.fecha { color:#777 }` (la fecha de "última actualización") sobre fondo claro — ratio ≈4.48:1, apenas por debajo del mínimo AA de 4.5:1 para texto normal. Cosmético, fix trivial (cambiar a `#707070` o similar).

**Lo que sigue igual que la auditoría anterior (16-ago), sin cambios:**
- 🟢 Secretos: 0 hallazgos reales. `_interno/github-pat.txt` sigue en `.gitignore` (línea 51) y nunca fue commiteado — no es una fuga.
- 🟢 Dominios externos: barrido completo de todos los `src`/`href` `https://` del repo — 0 dominios nuevos o sospechosos. Todos coinciden con la lista blanca conocida (familia Travelpayouts/`*.tpo.li`, CJ `jdoqocy.com`/`tkqlhce.com`, Viator, GetYourGuide, DiscoverCars, Pexels/Unsplash, Google Fonts/Tag Manager/AdSense, Facebook/Instagram, migración RD).
- 🟢 SEO técnico: 33 guías ES + portada + landing + página de campaña revisadas (37 páginas) — 0 sin title, 0 sin meta description, 0 sin H1/duplicado, 0 sin canonical (único caso sin canonical es `bio-link-el-quisqueyano.html`, que tiene `noindex` a propósito, así que no aplica). 0 títulos/descriptions duplicados. 47 páginas EN+FR revisadas — mismo resultado limpio, con `hreflang` presente en todas. `sitemap.xml` con 79 URLs. `robots.txt` limpio. `vuelos-navidad-nyc-rd.html` (landing de campaña nueva, commiteada) correctamente marcada `noindex` y fuera del sitemap, como corresponde.
- 🟢 Contraste WCAG (fuera del caso puntual arriba), zonas táctiles y foco visible: sin cambios, sigue en verde.
- 🟢 Web Interface Guidelines (Vercel): 0 `transition: all`, 0 `user-scalable=no`, 0 "..." literal en páginas nuevas/modificadas revisadas.
- 🟢 Voz humana: 0 frases genéricas de IA en las páginas nuevas (`contacto`, `privacidad`, `sobre-nosotros`, `terminos`, landing de Navidad).
- 🔵 Accesibilidad menor, nuevo: los `<script>` de los widgets de Travelpayouts (`tpemb.com`) no tienen forma de dar contexto por lectores de pantalla si el script no carga (por el bug de CSP de arriba, ahora mismo no cargan igual) — no es un fix separado, se resuelve solo al arreglar el CSP.

**Corrección sobre la nota del 16-sep:** esa sesión reportó que las guías nuevas de `Desktop\Quisqueya` "no están desplegadas" por no encontrarlas en `GitHub\quisqueyatravel` — eso fue un error de diagnóstico (usó la carpeta vieja como referencia). `Desktop\Quisqueya` sí es la que se despliega. Las guías que sí faltan por confirmar en vivo son justamente las 4 páginas de confianza sin commitear (arriba), no las guías de destinos (esas sí están publicadas desde hace semanas).

**Sigue igual que semanas anteriores (no es hallazgo nuevo, bloqueado en Venul):** 3 testimonios reales para `#resenas`, aprobación de "Booking.com LATAM" en CJ Affiliate.
